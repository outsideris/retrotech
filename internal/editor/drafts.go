...
// structured draft's description is derived from its intro, and the feed HTML
// from both description fields (see derive.go) — drafts are always new content,
// so there are no legacy bytes to preserve.
func (ds *DraftStore) Save(slug string, form EpisodeForm) error {
	...
	form.FeedDescription = deriveFeedDescription(form.Description, form.Description2)
