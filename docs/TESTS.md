...
| `internal/builder/feed_test.go` | `feed.go` | **피드 골든**: `BuildFeed` 출력이 `testdata/feed.golden.xml` 과 바이트 동일(휘발성 `lastBuildDate` 정규화)임을 전체 에피소드로 검증. **챕터**: 챕터 선언 시 `xmlns:podcast` + `<podcast:chapters>` + description 타임스탬프 줄 병기, 미선언 시 피드 무변화(네임스페이스·태그 부재). **description HTML**(`descriptionHTML`): 줄바꿈→`<br/>`·빈 줄→`<p>`·연속 빈 줄 병합·앞뒤 개행 제거·맨 URL→`<a href>`·문장부호는 앵커 밖·`&`/`<`/`>` 이스케이프(URL 쿼리 `&` 포함)·빈 입력→빈 결과, 그리고 아이템 description 이 CDATA 안에서 이스케이프되지 않은 HTML 로 나가는지 |
