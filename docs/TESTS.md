...
| `internal/editor/editor_test.go` | `editor.go` | HTTP 핸들러 전수(httptest): 목록·생성(201, 파일 기록)·중복(409)·조회(필드 보존)·수정(200)·삭제(204)·재조회(404), 에러 코드(미존재 404·잘못된 슬러그 400·깨진 JSON 400), `POST /preview`(text/html·제목·레퍼런스 링크 렌더), UI/자산 서빙(임베드 index·app.js + `Cache-Control: no-store`, `public/` styles.css, `/`→302), `New` 가 content/episodes 없는 repo 거부 |
