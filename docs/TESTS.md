...
| `internal/editor/compose_test.go` | `compose.go`·`form.go` | **에디터 라운드트립 안전망**: `content/episodes/` 23편 전수를 폼으로 변환→재합성→재파싱해 ① 프론트매터 값 동일(`reflect.DeepEqual`) ② 구조화 본문 바이트 동일 ③ 재합성 에피소드의 `BuildFeed` 가 원본과 **바이트 동일**(피드 골든 계약 보호). 추가: 블록 스칼라 chomping(strip/clip/keep) 값 보존, `parseLinkItem` 가역성(괄호 포함 URL·plain 텍스트), 빈 섹션 생략(레퍼런스 없는 회차), **챕터 라운드트립**(`TestChaptersSurviveFrontmatterRoundTrip`): 합성한 `chapters:` 가 값 동일하게 재파싱되는지 — bare `"02:41"` 의 sexagesimal 해석(→161) 방지용 강제 따옴표, 비ASCII 시작·`": "`·`#`·따옴표 포함 제목, `HH:MM:SS`, 예약어(`true`) 제목, `StartSeconds()` 초 변환. 챕터 없는 회차가 빈 `chapters:` 키를 얻지 않는지(`TestComposeOmitsChaptersWhenEmpty`) |
...
| `internal/editor/assist/assist_test.go` | `assist.go` | AI CLI 제공자: `Providers()`/`Find`(claude/codex/gemini), claude envelope 파싱(텍스트+**텔레메트리**: duration/cost/tokens·is_error·raw fallback·빈), codex JSONL 파싱(agent_message + usage 토큰·비JSON 라인 무시·error 이벤트), `resolveBinary`(PATH 이름 후보 + fallback 경로). (모델/effort 검증은 `catalog_test.go` 로 분리. 실제 CLI exec 은 환경 의존이라 미검증) |
| `internal/editor/assist/catalog_test.go` | `catalog.go` | **모델 카탈로그와 검증**: 항목 형식(빈 id/label 금지 — `""` 는 UI 의 "(기본)" 자리, id·label 중복 금지, effort 는 알려진 어휘 low/medium/high/xhigh/max/ultra 안에서 중복 없이), **현재 모델 목록 고정**(claude = Opus 5·Sonnet 5·Fable 5.1·Haiku 4.5, codex = GPT-5.6 Sol·Terra·Luna·GPT-5.5, gemini 없음 — CLI 갱신 시 같이 바뀌어야 하는 의도적 핀), `Validate`(기본 모델은 provider 합집합 기준·그 밖의 값 거부, 모델별 상한 초과 거부(Sonnet 5 에 ultra, GPT-5.5 에 max, Luna 에 ultra), effort 안 받는 모델에 effort 거부, 은퇴한 별칭/모델(`opus`·`gpt-5-codex`)·은퇴한 effort(`minimal`)·이 CLI 가 못 쓰는 `gpt-6-astra`·숨김 모델(`gpt-reserve`) 거부, 오류가 provider 이름으로 시작), **거부가 CLI 실행 전에 일어남**(PATH 를 가짜 CLI 만 있는 디렉터리로 바꾸고, 실행되면 남는 흔적이 없음을 확인 — 흔적은 셸 리다이렉션으로 남긴다. PATH 를 바꾼 뒤에는 `touch` 같은 외부 명령이 해석되지 않아 가드가 헛돌기 때문). 이 가드는 "거부가 exec 보다 먼저"만 증명하며, 실제 CLI 호출 자체는 미검증이다 |
...
- **Assist 사이드바 JS**(`app.js` 의 `renderTuning`/`renderEfforts`/`unionEfforts`/`fillSelect`): 모듈로 분리되어 있지 않아 단위 테스트가 없다. 모델↔effort 연동은 실행 중인 앱을 브라우저로 직접 확인한다(2026-09-20 확인: 모델별 effort 목록, effort 없는 모델의 노브 숨김, gemini 의 줄 숨김, 저장값 복원/폐기). Go 쪽은 DOM id 계약만 지킨다.
