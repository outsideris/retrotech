...
- [x] **GitHub Sponsors iframe 지연 로드(2026-06-16).** 푸터 iframe 에 `loading="lazy"` 추가 — 접힘 아래라 초기 로드에서 github.com 요청을 미룸.
...
- [ ] **에디터 앱 챕터 지원.** `EpisodeForm`/`composeFrontmatter` 가 `chapters:` 를 모른다 — 챕터 있는 에피소드를 에디터로 저장하면 **유실**된다. 폼 필드 추가 + 라운드트립 테스트에 챕터 케이스 포함 전까지, 챕터 회차는 에디터로 편집하지 말 것.
