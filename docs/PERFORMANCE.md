...
로컬 Lighthouse 측정에 더해, CI에서 회귀를 자동 차단한다(두 층위).

1. **Go 마크업 불변식** (`internal/builder/a11y_perf_test.go`, 기존 `test` 잡) — 브라우저 없이 결정적. Lighthouse 점수를 만드는 마크업을 전 페이지 타입(home/episodes/episode/404)에 대해 단언: 제목 계층 건너뜀 없음·단일 h1, 모든 `img` `alt`, 단일 `role="main"`+skip 링크, 다크 토글 키보드 조작, `html lang`·`title`, iframe `loading="lazy"`, `height="0"` 부재(CLS), 커버 preload 위치(home·404)·`fetchpriority`, 커버 `width/height`.
2. **Lighthouse CI** (`.lighthouserc.json`, 새 `lighthouse` 잡) — 빌드 후 `cmd/serve` 를 자동 기동해 `/`·`/episodes`·`/episodes/2g` 를 실제 감사. **접근성/SEO/Best-Practices=100 은 하드 게이트**(`error`, 위반 시 CI 실패). **성능·CLS 는 경고**(`warn`) — localhost 절대 타이밍이 비대표적이라 빌드를 깨지 않되 회귀는 드러낸다(결정적 마크업은 1층이 이미 보장).

로컬 재현: `npx @lhci/cli@0.14.x autorun` (검증 시점 측정: 세 URL 모두 a11y/SEO/BP 1.00, 성능 0.93–1.00, CLS 0.000).
